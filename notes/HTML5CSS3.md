...
-  ### img:
   1. img属于自结束标签，替换元素，引入图片的路径规则和超链接一致
-  ### audio：
   1. 引入音频文件，属性controls/autoplay/loop，可用src属性或者内部source标签配合内部src指定文件（可指定多个文件，在老版解决兼容性问题）。可使用embed标签支持IE8。video标签引入视频使用方法类似audio
-  ### 表格 
   1. table /tr 行/td 单元格
...
- ### 其他设置
    ...
    2. 轮廓和圆角：
       ...
       2. box-shadow：设置元素的阴影，不影响页面布局
          ...
          2. 第二个参数：垂直偏移量，设置阴影的垂直位置，正数向下负数向上
          3. 第三个参数：阴影的模糊半径，值越大阴影越模糊
          4. 第三个参数：阴影的颜色
...
- ### 字体&背景
    ...
    2. #### 图标字体（icofont）：
       1. 可将图标设置为字体，通过font-face的形式对字体进行引入（例如fontawesome.com或阿里的icofont）
       2. 将css和webfonts移入项目，通过css引入后使用<i class="用的图标的类名"></i>
       3. 通过伪元素设置图标字体:
          1. 通过before找到要设置图标的元素
          2. 在content中设置字体编码
          3. 设置字体样式：font-family和font-weight
       4. 通过实体设置：<i class="图标类名">&#x字体图标编码;</i>
    3. #### 行高：
        1. 文字占有的实际高度，可通过line-height设置，可使用一个大小或一个整数（字体的指定倍数）
        2. 字体框：就是字体存在的格子，设置font-size实际上就是在设置字体框的高度
        3. 行高会在字体框的上下平均分配
        4. 所以可以将行高设置为和高度相同的值，则单行文字会在一个元素中垂直居中
        5. 行高常用来设置文字行间距，行间距=行高-字体大小
...
- ### 动画：可自动触发动态效果
   ...
   ```
   1. animation-name(对当前元素生效的关键帧名):test
   2. animation-duration: 动画持续时间
   3. amination-delay:动画的延时
   4. amination-timing-function: 动画的时序函数
   5. animation-iteration-count: 动画执行次数，可选具体次数或infinite
   6. animation-direction: 动画运行方向，默认nomal，代表每次都从from到to运行/reverse 反向执行/alternate 重复执行动画时往复执行/alternate reverse
   7. animation-play-state: 设置动画执行状态，默认running/paused
   8. animation-fill-mode: 填充模式，默认none，动画执行完毕元素回到原来位置/forwards动画执行完毕元素会停在动画结束位置/backwards动画延时等待时，元素处于开始位置/both
---
